package services
{
	...
	public class NightscoutServiceEnhanced
	{
		/* Constants */
		...
		public function NightscoutServiceEnhanced()
		{
		...
		public static function init():void
		{
			...
			Trace.myTrace("NightscoutServiceEnhanced.as", "Service started!");
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "in getInitialGlucoseReadings.");
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "in onUploadGlucoseReadingsComplete.");
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Glucose reading upload was successful.");
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Error uploading glucose reading.");
			}
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "in getInitialCalibrations.");
			...
			Trace.myTrace("NightscoutServiceEnhanced.as", "Initial calibrations to upload: " + activeCalibrations.length);
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "in onUploadCalibrationsComplete.");
			...
			if (response.indexOf(BlueToothDevice.name) != -1)
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Calibration upload was successful.");
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Error uploading calibration.");
			}
		}
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "onUploadVisualCalibrationsComplete");
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Visual calibration upload was successful!");
				activeVisualCalibrations.length = 0;
			}
			else
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Error uploading visual calibration!");
			}
		}
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "in getSensorStart.");
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "onUploadSensorStartComplete");
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Sensor start uploaded successfuly");
				activeSensorStarts.length = 0;
			}
			else
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Error uploading sensor start!");
			}
		}
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "testNightscoutCredentials called. External call = " + externalCall);
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Can't check NS credentials. No Internet connection!");
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "onTestCredentialsComplete called");
			...
			if (response != "")
			{
				...
				{
					Trace.myTrace("NightscoutServiceEnhanced.as", "NS Authentication failed! Careportal not enabled.");
				...
				else
				{
					...
					{
						Trace.myTrace("NightscoutServiceEnhanced.as", "NS Authentication successful! Activating service");
					...
					{
						Trace.myTrace("NightscoutServiceEnhanced.as", "Authentication failed! Wrong api secret?");
						Trace.myTrace("NightscoutServiceEnhanced.as", "Error:", responseInfo.status + " " + responseInfo.message);
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Authentication failed! URL not found.");
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "Service activated!");
			serviceActive = true;
			setupNightscoutProperties();
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "Service deactivated!");
			serviceActive = false;
		...
		private static function onConnectionFailed(error:Error, mode:String):void
		{
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "In onConnectionFailed. Error uploading glucose readings. Error: " + error.message);
				syncGlucoseReadingsActive = false;
			}
			else if (mode == MODE_CALIBRATION)
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "In onConnectionFailed. Error uploading calibrations. Error: " + error.message);
				syncCalibrationsActive = false;
			}
			else if (mode == MODE_VISUAL_CALIBRATION)
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "In onConnectionFailed. Error uploading visual calibrations. Error: " + error.message);
				syncVisualCalibrationsActive = false;
			}
			else if (mode == MODE_SENSOR_START)
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "in onConnectionFailed. Error uploading sensor start event. Error: " + error.message);
				syncSensorStartActive = false;
			}
			else if (mode == MODE_TEST_CREDENTIALS)
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "in onConnectionFailed. Can't make connection to the server. Error: " +  error.message);
				externalAuthenticationCall = false;
			}
		}
		...
		private static function onSettingChanged(e:SettingsServiceEvent):void
		{
			...
			if (e.data == CommonSettings.COMMON_SETTING_NIGHTSCOUT_ON) 
			{
				if (CommonSettings.getCommonSetting(CommonSettings.COMMON_SETTING_NIGHTSCOUT_ON) == "true")
				{
					...
					{
						Trace.myTrace("NightscoutServiceEnhanced.as", "in onSettingChanged, activating service");
						activateService();
					}
				}
				else
				{
					Trace.myTrace("NightscoutServiceEnhanced.as", "in onSettingChanged, deactivating service.");
					deactivateService();
				}
			}
			else if (e.data == CommonSettings.COMMON_SETTING_API_SECRET || e.data == CommonSettings.COMMON_SETTING_AZURE_WEBSITE_NAME) 
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "in onSettingChanged, restesting credentials");
				deactivateService();
				setupNightscoutProperties();
				testNightscoutCredentials();
			}
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "in onSettingChanged, uploading new sensor.");
				getSensorStart();
			}
		}
		...
		private static function onNetworkChange( event:NetworkInfoEvent ):void
		{
			...
			{
				Trace.myTrace("NightscoutServiceEnhanced.as", "Network is reachable again. Calling resync.");
				resync();
			}
			else
		...
		{
			Trace.myTrace("NightscoutServiceEnhanced.as", "App in foreground. Calling resync.");
			resync();
		}
