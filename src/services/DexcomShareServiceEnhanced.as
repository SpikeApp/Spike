package services
{
	...
	public class DexcomShareServiceEnhanced extends EventDispatcher
	{
		/* Instance */
		public static const instance:DexcomShareServiceEnhanced = new DexcomShareServiceEnhanced();
		...
		public function DexcomShareServiceEnhanced()
		{
			if (instance != null)
		...
		public static function init():void
		{
			...
			Trace.myTrace("DexcomShareServiceEnhanced.as", "Service started!");
		...
		{
			Trace.myTrace("DexcomShareServiceEnhanced.as", "onTestCredentialsComplete called");
			...
			if (response != "")
			{
				if (response.indexOf("Code") == -1)
				{
					...
					Trace.myTrace("DexcomShareServiceEnhanced.as", "Authentication successful! Session ID: " + dexcomShareSessionID);
				...
				{
					Trace.myTrace("DexcomShareServiceEnhanced.as", "Authentication error! Trying to parse error...");
					...
					if (responseInfo != null)
					{
						...
						Trace.myTrace("DexcomShareServiceEnhanced.as", "Error parsed successfully! Error code: " + errorCode);
						...
						{	
							if (errorCode == "SSO_AuthenticateAccountNotFound") 
							{
								if (externalAuthenticationCall) 
								{
									Trace.myTrace("DexcomShareServiceEnhanced.as", "Alerting user of invalid account.");
							...
							else if (errorCode == "SSO_AuthenticatePasswordInvalid") 
							{
								if (externalAuthenticationCall) 
								{
									Trace.myTrace("DexcomShareServiceEnhanced.as", "Alerting user of invalid password.");
							...
							else if (errorCode == "SSO_AuthenticateMaxAttemptsExceeed") 
							{
								...
								{
									Trace.myTrace("DexcomShareServiceEnhanced.as", "Alerting user of max authentication attempts exceeded.");
						...
						{
							Trace.myTrace("DexcomShareServiceEnhanced.as", "There's no error code in server's response. Aborting!");
						}
						
					}
					else
					{
						Trace.myTrace("DexcomShareServiceEnhanced.as", "Unable to parse error!");
						
						if (externalAuthenticationCall)
						{
							//Alert User
							Trace.myTrace("DexcomShareServiceEnhanced.as", "Displaying generic error message to the user.");
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "Authentication error! Error: Service unavailable");
				if (externalAuthenticationCall)
				{
					//Alert User
					AlertManager.showSimpleAlert(
		...
		private static function getInitialGlucoseReadings():void
		{
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "Number of initial glucose readings to upload: " + activeGlucoseReadings.length);
				syncGlucoseReadings();
			}
			else
				Trace.myTrace("DexcomShareServiceEnhanced.as", "No initial glucose readings!");
		}
		...
		{
			Trace.myTrace("DexcomShareServiceEnhanced.as", "in onUploadGlucoseReadingsComplete.");
			...
			if (response == "")
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "Glucose reading(s) uploaded successfully!");
				
				glucoseUploadSuccessfull();
			}
			else
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "Error uploading glucose reading(s)! Trying to parse error...");
				...
				if (responseInfo != null)
				{
					...
					{
						Trace.myTrace("DexcomShareServiceEnhanced.as", "There were duplicate glucose entries. Not problematic. Treating as successful!");
					...
					{
						Trace.myTrace("DexcomShareServiceEnhanced.as", "No action taken. Error code is unknown.");
					} 
				}
		...
		private static function onBgreadingReceived(e:TransmitterServiceEvent):void 
		{
			...
			activeGlucoseReadings.push(createGlucoseReading(BgReading.lastNoSensor()));
		 ...
		 * ASSIGN RECEIVER
		 */
		private static function assignReceiver(receiverID:String):void
		{
			Trace.myTrace("DexcomShareServiceEnhanced.as", "assignReceiver called!");
		...
		{
			Trace.myTrace("DexcomShareServiceEnhanced.as", "onAssignReceiverResponse called!");
			...
			if (response.indexOf("Code") == -1)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "Receiver assigned successfully!!");
				getInitialGlucoseReadings();
				
				if (BackgroundFetch.appIsInForeground())
				{
					Trace.myTrace("DexcomShareServiceEnhanced.as", "Notifying user...");
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "Error assigning receiver! Trying to parse error code");
				...
				if (responseInfo != null)
				{
					...
					Trace.myTrace("DexcomShareServiceEnhanced.as", "Error parsed successfully! Error code: " + errorCode + " | Message: " + responseInfo.Message);
		...
		{
			Trace.myTrace("DexcomShareServiceEnhanced.as", "Service activated!");
			serviceActive = true;
		...
		{
			Trace.myTrace("DexcomShareServiceEnhanced.as", "Service deactivated!");
			serviceActive = false;
		...
		private static function parseDexcomError(response:String, nextAction:Function):Object
		{
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "Can't parse server response! Error: " + error.message);
				
				var now:Number = (new Date()).valueOf();
				if (now - timeStampOfLastLoginAttemptSinceJSONParsingErrorReceived > RETRY_TIME_FOR_SERVER_ERRORS) 
				{
					Trace.myTrace("DexcomShareServiceEnhanced.as", "Trying new login...");
				...
				{
					Trace.myTrace("DexcomShareServiceEnhanced.as", "Not trying new login...");
				}
			}
		...
		private static function onConnectionFailed(error:Error, mode:String):void
		{
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Error uploading glucose readings. Error: " + error.message);
				syncGlucoseReadingsActive = false;
			}
			else if (mode == MODE_TEST_CREDENTIALS)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Authentication failed! Error: " + error.message);
				externalAuthenticationCall = false;
			}
			else if (mode == MODE_REMOTE_MONITORING_SESSION)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Remote monitoring session request failed! Error: " + error.message);
			}
			else if (mode == MODE_ASSIGN_RECEIVER)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't assign new receiver! Error: " + error.message);
			}
			else if (mode == MODE_LIST_FOLLOWERS)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't list followers! Error: " + error.message);
				instance.dispatchEvent(new DexcomShareEvent(DexcomShareEvent.LIST_FOLLOWERS, null));
			}
			else if (mode == MODE_DELETE_FOLLOWER)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't delete follower! Error: " + error.message);
				instance.dispatchEvent(new DexcomShareEvent(DexcomShareEvent.DELETE_FOLLOWER, null));
			}
			else if (mode == MODE_CREATE_CONTACT)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't create contact! Error: " + error.message);
				instance.dispatchEvent(new DexcomShareEvent(DexcomShareEvent.CREATE_CONTACT, null));
			}
			else if (mode == MODE_INVITE_FOLLOWER)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't invite follower! Error: " + error.message);
				instance.dispatchEvent(new DexcomShareEvent(DexcomShareEvent.INVITE_FOLLOWER, null));
			}
			else if (mode == MODE_GET_FOLLOWER_INFO)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't get follower info! Error: " + error.message);
				instance.dispatchEvent(new DexcomShareEvent(DexcomShareEvent.GET_FOLLOWER_INFO, null));
			}
			else if (mode == MODE_GET_FOLLOWER_ALARMS)
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't get follower alarms! Error: " + error.message);
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't change follower name! Error: " + error.message);
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't change follower permissions! Error: " + error.message);
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't enable follower sharing! Error: " + error.message);
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "in onConnectionFailed, Can't disable follower sharing! Error: " + error.message);
		...
		private static function onSettingChanged(e:SettingsServiceEvent):void
		{
			...
			if (e.data == CommonSettings.COMMON_SETTING_DEXCOM_SHARE_ON) 
			{
				...
				{
					Trace.myTrace("DexcomShareServiceEnhanced.as", "onSettingChanged called, deactivating service.");
					deactivateService();
					return;
				}
				else
				{
					...
					{
						Trace.myTrace("DexcomShareServiceEnhanced.as", "onSettingChanged called, user or password are blank, deactivating service.");
						deactivateService();
						return;
					}
					...
					if (!serviceActive)
					{
						Trace.myTrace("DexcomShareServiceEnhanced.as", "onSettingChanged called, activating service.");
						activateService();
					}
				}
			}
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "onSettingChanged called, account name, password or URL changed. Resetting session ID and login again.");
				dexcomShareSessionID = "";
		...
		{
			Trace.myTrace("DexcomShareServiceEnhanced.as", "App in foreground. Calling resync.");
			resync();
		}
		...
		private static function onNetworkChange( event:NetworkInfoEvent ):void
		{
			...
			{
				Trace.myTrace("DexcomShareServiceEnhanced.as", "Network is reachable again. Calling resync.");
				resync();
			}
			else
